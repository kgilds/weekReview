...
ui <- page_navbar(
  ...
  nav_panel(
    ...
    layout_columns(
      card(
        ...
        checkboxInput("notes_captured", "Notes from the week captured (notebooks, meeting minutes, loose notes)")
      ),
      card(
        ...
        checkboxInput("approvals_pending", "Any approvals, reviews, or sign-offs pending")
      )
    )
  ),
  ...
  nav_panel(
    ...
    layout_columns(
      ...
      card(
        ...
        textAreaInput("conversations_text", "Conversations to schedule:", rows = 3)
      )
    ),
    layout_columns(
      card(
  ...
  nav_panel(
    ...
    layout_columns(
      ...
      card(
        ...
        checkboxInput("no_meeting_blocked", "No-meeting time blocked (if possible)")
      )
    )
  ),
  ...
  nav_panel(
    ...
    card(
      ...
      textAreaInput("stop_start_continue", "What should I stop / start / continue?", rows = 3),
      textAreaInput("successful_week", "What would make next week feel successful?", rows = 3)
    )
  ),
  ...
  nav_panel(
    ...
    card(
      ...
      textInput("one_thing", "Choose 'One Thing' for next week:", placeholder = "Your most important focus"),
      checkboxInput("gratitude", "Close out with gratitude"),
      textAreaInput("gratitude_text", "What are you grateful for this week?", rows = 3),
      br(),
      layout_columns(
        actionButton("save_review", "Save Weekly Review", class = "btn-primary btn-lg"),
...
server <- function(input, output, session) {
  ...
  completion_progress <- reactive({
    ...
    get_clear <- sum(input$chat_cleared, input$inbox_emptied, input$notes_captured,
                     input$promises_made, input$followups_owed, input$approvals_pending)
    
    # Get Current section (14 items)
    get_current <- sum(input$last_week_wins, input$missed_commitments, input$prep_needed,
                       input$conversations_schedule, input$review_projects, input$update_status,
                       input$identify_risks, input$stuck_items, input$checkins_needed,
                       input$people_nudge, input$staff_support, input$consultant_direction,
                       input$bottlenecks_clear, input$decisions_needed)
    
    # Get Intentional section (5 scheduling items)
    get_intentional <- sum(input$deep_work_scheduled, input$oneones_scheduled,
                           input$admin_scheduled, input$travel_scheduled, input$no_meeting_blocked)
    
    # Get Aligned section (5 items)
    get_aligned <- sum(input$role_goals, input$team_goals, input$program_goals,
                       input$emerging_risks, input$opportunities)
    
    # Finalize section (2 items)
    finalize <- sum(input$system_reviewed, input$gratitude)
    
    total_checks <- get_clear + get_current + get_intentional + get_aligned + finalize
    total_possible <- 32
  ...
  output$summary <- renderText({
    ...
    summary_text <- paste0(
      ...
      "Overall Completion: ", progress$percentage, "% (", progress$total, "/32 items)\n\n",
      "Section Breakdown:\n",
      "• Get Clear: ", progress$get_clear, "/6 items\n",
      "• Get Current: ", progress$get_current, "/14 items\n", 
      "• Get Intentional: ", progress$get_intentional, "/5 items\n",
      "• Get Aligned: ", progress$get_aligned, "/5 items\n",
      "• Finalize: ", progress$finalize, "/2 items\n\n"
    )
    
    if(input$one_thing != "") {
      summary_text <- paste0(summary_text, "ONE THING for next week: ", input$one_thing, "\n\n")
    }
  ...
  generate_report_content <- reactive({
    ...
    report <- paste0(
      ...
      paste(rep("-", 60), collapse = ""), "\n",
      "Capture & Clean Up:\n",
      "  ", checkbox_status(input$chat_cleared), " Chat messages cleared\n",
      "  ", checkbox_status(input$inbox_emptied), " Physical inbox emptied\n",
      "  ", checkbox_status(input$notes_captured), " Notes from week captured\n\n",
      
      "Loose Ends:\n",
      "  ", checkbox_status(input$promises_made), " Promises made to others\n",
      "  ", checkbox_status(input$followups_owed), " Follow-ups owed to others\n",
      "  ", checkbox_status(input$approvals_pending), " Approvals/reviews/sign-offs pending\n\n",
      
      "GET CURRENT (Review)\n",
      paste(rep("-", 60), collapse = ""), "\n",
      "Calendar Review - Last Week:\n",
      "  ", checkbox_status(input$last_week_wins), " Wins\n",
      add_content("Wins Details", input$wins_text, TRUE),
      "  ", checkbox_status(input$missed_commitments), " Missed commitments\n",
      add_content("Missed Commitments Details", input$missed_text, TRUE),
      
      "Calendar Review - Next 2 Weeks:\n",
      "  ", checkbox_status(input$prep_needed), " Prep needed\n",
      add_content("Prep Details", input$prep_text, TRUE),
      "  ", checkbox_status(input$conversations_schedule), " Conversations to schedule\n",
      add_content("Conversations Details", input$conversations_text, TRUE),
      
      "Projects / Areas:\n",
      "  ", checkbox_status(input$review_projects), " Review all active projects\n",
      "  ", checkbox_status(input$update_status), " Update status and next actions\n",
      "  ", checkbox_status(input$identify_risks), " Identify projects at risk\n",
      add_content("Project Notes", input$projects_notes, TRUE),
      
      "Waiting For:\n",
      "  ", checkbox_status(input$stuck_items), " Items that are stuck\n",
      "  ", checkbox_status(input$checkins_needed), " Check-ins needed\n",
      "  ", checkbox_status(input$people_nudge), " People to nudge\n",
      add_content("Waiting For Notes", input$waiting_notes, TRUE),
      
      "Team / Leadership Responsibilities:\n",
      "  ", checkbox_status(input$staff_support), " Staff needing support\n",
      "  ", checkbox_status(input$consultant_direction), " Consultants needing direction\n",
      "  ", checkbox_status(input$bottlenecks_clear), " Bottlenecks to clear\n",
      "  ", checkbox_status(input$decisions_needed), " Decisions only you can make\n",
    ...
    # Add key outcomes
    outcomes <- c(input$outcome1, input$outcome2, input$outcome3, input$outcome4, input$outcome5)
    for(i in 1:5) {
    ...
    report <- paste0(
      ...
      "Block Your Week:\n",
      "  ", checkbox_status(input$deep_work_scheduled), " Deep work blocks scheduled\n",
      "  ", checkbox_status(input$oneones_scheduled), " 1:1s & check-ins scheduled\n",
      "  ", checkbox_status(input$admin_scheduled), " Administrative work scheduled\n",
      "  ", checkbox_status(input$travel_scheduled), " Travel / field time scheduled\n",
      "  ", checkbox_status(input$no_meeting_blocked), " No-meeting time blocked\n\n",
      
      "GET ALIGNED (Bigger Picture)\n",
      paste(rep("-", 60), collapse = ""), "\n",
      "Alignment Check:\n",
      "  ", checkbox_status(input$role_goals), " Alignment with role goals\n",
      "  ", checkbox_status(input$team_goals), " Alignment with team goals\n",
      "  ", checkbox_status(input$program_goals), " Alignment with program goals\n",
      "  ", checkbox_status(input$emerging_risks), " Emerging risks\n",
      "  ", checkbox_status(input$opportunities), " Opportunities\n",
      ...
      add_content("What would make next week feel successful", input$successful_week, TRUE),
      
      "FINALIZE\n",
      paste(rep("-", 60), collapse = ""), "\n",
      "  ", checkbox_status(input$system_reviewed), " System reviewed one final time\n",
      "  ", checkbox_status(input$gratitude), " Close out with gratitude\n\n"
    )
    
    if(nchar(trimws(input$one_thing)) > 0) {
      report <- paste0(report, "ONE THING for next week:\n  ", input$one_thing, "\n\n")
    }
